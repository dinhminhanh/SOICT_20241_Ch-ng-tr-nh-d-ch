...
void compileCondition(void) {
  ...
  switch (op) {
  case SB_EQ: eat(SB_EQ); genEQ(); break;
  case SB_NEQ: eat(SB_NEQ); genNE(); break;
  case SB_LE: eat(SB_LE); genLE(); break;
  case SB_LT: eat(SB_LT); genLT(); break;
  case SB_GE: eat(SB_GE); genGE(); break;
  case SB_GT: eat(SB_GT); genGT(); break;
  default:
...
Type* compileIndexes(Type* arrayType) {
  ...
  while (lookAhead->tokenType == SB_LSEL) {
    ...
    genAD(); // Sinh mã tính địa chỉ phần tử
